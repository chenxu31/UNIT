...
def main(logger, opts):
    ...
    test_data_t, test_data_s = common_ixi.load_test_data(opts.data_dir, "test")
    ...
    with torch.no_grad():
        for i in range(len(test_data_s)):
            ...
            if opts.output_dir:
                common_ixi.save_nii(test_ts, os.path.join(opts.output_dir, "syn_%d.nii.gz" % i))
            ...
            msg_detail += "  %d_psnr: %f  %d_ssim: %f  %d_mae: %f\n" % (i, ts_psnr, i, ts_ssim, i, ts_mae)
